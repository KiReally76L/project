...
import Russuian from '../../components/json/KartRussian.json'
...
function List() {
  ...
  const onClickSlider = (direct) => {
    ...
    if (direct === 'left' && sliderNumber > 0) {
      ...
      listRef.current.style.transform = `translateX(${220 + directe}px)`
    } 
    if (direct === 'right' && sliderNumber < 5) {
      setSliderNumber(sliderNumber + 1)
      listRef.current.style.transform = `translateX(${-270 + directe}px)`
    }
  }
  return (
    <div className='list'>
      <span className="listTitle">Русские комедии</span>
