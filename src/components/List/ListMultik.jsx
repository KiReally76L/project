...
import Multik from '../../components/json/KartMultik.json'
...
function ListMultik() {
  ...
  const onClickSlider = (direct) => {
    ...
    if (direct === 'left' && sliderNumber > 0) {
      ...
      listRef.current.style.transform = `translateX(${650 + directe}px)`
    } 
    if (direct === 'right' && sliderNumber < 5) {
      setSliderNumber(sliderNumber + 1)
      listRef.current.style.transform = `translateX(${-695 + directe}px)`
    }
  }
  return (
    <div className='list'>
    <span className="listTitle">Мультфильмы</span>
