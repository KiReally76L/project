...
import KartDh from '../../components/json/KartDhVsev.json'
...
function ListKartDh() {
  ...
  const onClickSlider = (direct) => {
    ...
    if (direct === 'left' && sliderNumber > 0) {
      ...
      listRef.current.style.transform = `translateX(${420 + directe}px)`
    } 
    if (direct === 'right' && sliderNumber < 5) {
      setSliderNumber(sliderNumber + 1)
      listRef.current.style.transform = `translateX(${-500 + directe}px)`
    }
  }
  return (
    <div className='list'>
    <span className="listTitle">Жанры</span>
